...
$whereClauses = [];
$bindings = [];
$selectClause = "SELECT images.*, COUNT(favorites.id) AS favorite_count";
$fromClause = "FROM images";
$joinClause = " LEFT JOIN favorites ON images.id = favorites.image_id";
...
// Total count query - do not count DISTINCT images.id if grouping, use COUNT(DISTINCT images.id) for accurate pagination
$totalQueryString = "SELECT COUNT(DISTINCT images.id) " . $fromClause . $joinClause . $sqlWhere;
...
$imagesQueryString = $selectClause . " " . $fromClause . $joinClause . $sqlWhere . 
                     " GROUP BY images.id ORDER BY favorite_count DESC LIMIT :page_limit OFFSET :page_offset";
