...
// Group the groups by the last character and sort them in descending order
$groupedgroups = [];
foreach ($groupCounts as $group => $count) {
  $lastChar = strtoupper(mb_substr($group, -1, 1));
  $groupedgroups[$lastChar][$group] = $count;
}

krsort($groupedgroups); // Sort groups by last character in descending order

// Get groups for the current category and sort them in descending order
$currentgroups = isset($groupedgroups[$category]) ? $groupedgroups[$category] : [];
arsort($currentgroups); // Sort groups within the category in descending order
