...
// Fetch user records and count followers
$query = "SELECT users.id, users.email, users.artist, users.pic, COUNT(f.following_email) AS followers_count
          FROM users 
          LEFT JOIN following f ON users.email = f.following_email
          GROUP BY users.id, users.email, users.artist, users.pic
          ORDER BY followers_count DESC, users.id DESC 
