        ...
        <?php if (!empty($tagCounts)): ?>
          <?php foreach ($tagCounts as $tag => $count): ?>
            <?php
              ...
              $escapedCount = $count;
            ?>
            <a class="btn bg-body-secondary link-body-emphasis border-0 fw-medium d-flex justify-content-between align-items-center w-100 rounded mt-2 p-3" href="?id=<?= urlencode($id) ?>&by=<?php echo isset($_GET['by']) ? (strpos($_GET['by'], 'tagged_') === false ? 'tagged_' . $_GET['by'] : $_GET['by']) : 'newest'; ?>&tag=<?= urlencode($escapedTag) ?>">
