                    ...
                    <div class="modal-content border-0">
                      <div class="modal-body p-0">
                        ...
                        <div class="modal-body p-0">
                          ...
                          <div class="row g-0">
                            <div id="div1_<?php echo $imageP['id']; ?>" class="overflow-auto col-md-7 h-100 scrollable-div">
                              <div>
                                <a href="/image.php?artworkid=<?php echo $imageP['id']; ?>">
                                  <img class="object-fit-cover shadow lazy-load h-100 w-100" data-src="/thumbnails/<?php echo $imageP['filename']; ?>" alt="<?php echo $imageP['title']; ?>">
                                </a>
                              </div>
                            </div>
                            <div id="div2_<?php echo $imageP['id']; ?>" class="overflow-auto col-md-5 h-100 scrollable-div">
                              <div class="p-3">
                                <h5 class="text-center fw-bold my-4"><?= $imageP['title']; ?></h5>
                                ...
                                <div class="btn-group mt-2 w-100">
                                  <a class="btn btn-sm border-0" data-bs-toggle="collapse" href="#shareSection_<?php echo $imageP['id']; ?>" role="button" aria-expanded="false" aria-controls="collapseExample"><i class="bi bi-share-fill icon-stroke-1"></i></a>
                                  <button class="btn btn-sm fw-bold border-0"><i class="bi bi-bar-chart-line-fill"></i> <?php echo $imageP['view_count']?></button>
                                  <button class="btn btn-sm border-0" type="button" data-bs-toggle="collapse" data-bs-target="#collapseInfoDesktop_<?php echo $imageP['id']; ?>" aria-expanded="false" aria-controls="collapseExample"><i class="bi bi-info-circle-fill"></i></button>
                                  <button class="btn btn-sm border-0" type="button" data-bs-toggle="collapse" data-bs-target="#collapseDownload_<?php echo $imageP['id']; ?>" aria-expanded="false" aria-controls="collapseExample"><i class="bi bi-download icon-stroke-1"></i></button>
                                  <a href="/image.php?artworkid=<?php echo $imageP['id']; ?>" class="btn btn-sm border-0"><i class="bi bi-eye-fill"></i></a>
                                </div>
                                <div class="collapse" id="shareSection_<?php echo $imageP['id']; ?>">
                                ...
                                <div class="collapse mt-2" id="collapseInfoDesktop_<?php echo $imageP['id']; ?>">
                                  <div class="card rounded-4 container">
                                    ...
                                    <?php
                                      ...
                                      $imageP_folder_path = $_SERVER['DOCUMENT_ROOT'] . '/images/';
    
                                      // Calculate and display image size and dimensions for the main image
                                      $imageP_path = $imageP_folder_path . $imageP['filename'];
                                      $imageP_size = round(filesize($imageP_path) / (1024 * 1024), 2);
                                      $total_image_size += $imageP_size;
                                      list($width, $height) = getimagesize($imageP_path);
                                      echo "<div class='mb-3 row'>
                                              <label for='' class='col-sm-4 col-form-label text-nowrap fw-medium'>Image ID</label>
                                                <div class='col-sm-8'>
                                                  <input type='text' class='form-control-plaintext fw-bold' id='' value='" . $imageP['id'] . "' readonly>
                                                </div>
                                            </div>";
                                      ...
                                      echo "<div class='mb-3 row'>
                                              ...
                                              <div class='col-sm-8'>
                                                <input type='text' class='form-control-plaintext fw-bold' id='' value='{$imageP_size} MB' readonly>
                                              </div>
                                            </div>";
                                      ...
                                      echo "<p class='text-start fw-medium'><a class='text-decoration-none' href='" . $base_url . $imageP['filename'] . "'>View original image</a></p>";
                            
                                      // Assuming you have a separate query to fetch child images
                                      $child_images_result = $dbP->query("SELECT filename FROM image_child WHERE image_id = " . $imageP['id']);
    
                                      while ($child_image = $child_images_result->fetchArray()) {
                                        $child_imageP_path = $imageP_folder_path . $child_image['filename'];
                                ...
                                <div class="collapse mt-2" id="collapseDownload_<?php echo $imageP['id']; ?>">
                                  <a class="btn btn-primary fw-bold rounded-4 w-100" href="/download_images.php?artworkid=<?php echo $imageP['id']; ?>" target="_blank">
                                  ...
                                  <p class="fw-bold text-center container">
                                    <small>2. If you found download error or failed, <a class="text-decoration-none" href="/download_batch.php?artworkid=<?php echo $imageP['id']; ?>">click this link</a> for third option if download all images error or failed.</small>
                                  </p>
                                  <p class="fw-bold text-center container">
