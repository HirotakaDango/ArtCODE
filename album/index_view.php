...
$stmt = $db->prepare('SELECT images.*, album.album_name, image_album.id AS image_album_id FROM image_album INNER JOIN images ON image_album.image_id = images.id INNER JOIN album ON image_album.album_id = album.id WHERE image_album.album_id = :album_id AND image_album.email = :email ORDER BY images.view_count DESC LIMIT :limit OFFSET :offset');
