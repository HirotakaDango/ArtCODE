...
$stmt = $db->prepare('CREATE TABLE IF NOT EXISTS album (id INTEGER PRIMARY KEY, email TEXT, album_name TEXT, image_id INTEGER)');
$stmt->execute();
  ...
  <body>
    ...
    <form method="post" class="container">
      <div class="form-floating mb-2">
        <input type="text" class="form-control rounded-3 border text-secondary fw-bold border-4" id="album_name" name="album_name" maxlength="40">
    ...
    <h5 class="text-secondary fw-bold text-center mt-4 mb-3"><i class="bi bi-images"></i>My Album</h5>
    <?php
      // Connect to the SQLite database
      $db = new SQLite3('database.sqlite');
       ...
       $email = $_SESSION['email'];
       $stmt = $db->prepare('SELECT DISTINCT album_name FROM album WHERE email = :email ORDER BY id DESC');
       $stmt->bindValue(':email', $email, SQLITE3_TEXT);
          ...
          ?>
            <div class="col-6 col-md-4 col-lg-3 mb-4">
              <div class="card border-4 h-100">
                <a class="border-bottom display-1 mt-1" href="album_images.php?album=<?= urlencode($album_name) ?>"><i class="bi bi-images text-secondary"></i></a>
                <a class="mt-1 text-secondary text-decoration-none fw-bold" href="album_images.php?album=<?= urlencode($album_name) ?>"><?= $album_name ?></a>
                <header class="d-flex justify-content-center py-3">
                  <form method="post">
                    <input type="hidden" name="delete_album" value="<?= $album_name ?>">
                    <button class="btn btn-sm btn-danger me-1 fw-bold" onclick="return confirm('Are you sure?')" type="submit"><i class="bi bi-trash-fill"></i></button>
                  </form> 
                  <a class="btn btn-sm btn-primary ms-1 fw-bold" href="edit_album.php?album=<?php echo $album_name; ?>"><i class="bi bi-pencil-fill"></i></a>
                </header>
              </div>
            </div>
          <?php }
