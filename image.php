        ...
        <div class="cool-6">
          <div class="caard">
            <a href="images/<?php echo $filename; ?>">
              <img class="img-fluid art" src="thumbnails/<?php echo $filename; ?>" alt="<?php echo $image['title']; ?>" width="100%" height="auto">
            </a>
        ...
        <div class="cool-6">
          <div class="caard">
            <div class="me-2 ms-2 rounded img-thumbnail fw-bold">
              ...
              <small class="text-secondary fw-bold text-center">Uploaded at: <?php echo $image['date']; ?></small>
              <div class="mt-3">
              ...
              <?php if ($next_image): ?>
                <button class="btn btn-sm btn-primary fw-bold float-start mb-2 rounded-pill mt-1" onclick="location.href='image.php?filename=<?= $next_image['filename'] ?>'">
                  <i class="bi bi-arrow-left-circle-fill"></i> Next
                </button>
              <?php endif; ?> 
              <?php if ($prev_image): ?>
                <button class="btn btn-sm btn-primary fw-bold float-end mb-2 rounded-pill mt-1" onclick="location.href='image.php?filename=<?= $prev_image['filename'] ?>'">
                  Previous <i class="bi bi-arrow-right-circle-fill"></i>
                </button>
              <?php endif; ?>
              <form action="add_to_album.php" method="get">
                <input class="form-control" type="hidden" name="image_id" value="<?= $image_id ?>">
                <select class="form-select fw-bold text-secondary rounded-pill mb-2" name="album_name">
                  <option class="form-control" value="">Add to album:</option>
                    <?php
                      ...
                      $stmt = $db->prepare('SELECT album_name FROM album WHERE email = :email');
                      ...
                      while ($row = $results->fetchArray(SQLITE3_ASSOC)) {
                        ...
                        echo '<option value="' . htmlspecialchars($album_name) . '">' . htmlspecialchars($album_name) . '</option>';
                      }
