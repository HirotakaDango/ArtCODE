...
function getAllImageFilenames($db, $filename) {
  $stmt = $db->prepare("SELECT filename FROM images WHERE id = :filename");
  $stmt->bindParam(':filename', $filename);
  $stmt->execute();
  $images = $stmt->fetchAll(PDO::FETCH_COLUMN);

  $stmt = $db->prepare("SELECT filename FROM image_child WHERE image_id = :filename");
  $stmt->bindParam(':filename', $filename);
  $stmt->execute();
...
$all_images = getAllImageFilenames($db, $filename);

// Create a ZIP file and add the images to it
$zip = new ZipArchive();

// Get the ID, title, and artist from the images and users tables using JOIN
$stmt = $db->prepare("SELECT i.id, i.title, REPLACE(u.artist, ' ', '_') AS artist FROM images i JOIN users u ON i.email = u.email WHERE i.id = :filename");
$stmt->bindParam(':filename', $filename);
$stmt->execute();
