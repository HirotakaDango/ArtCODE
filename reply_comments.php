...
if (isset($_POST['reply_comment_id'], $_POST['reply'])) {
  ...
  $reply = nl2br(filter_var($_POST['reply'], FILTER_SANITIZE_FULL_SPECIAL_CHARS, FILTER_FLAG_STRIP_LOW));
  ...
  header('Location: reply_comments.php?comment_id=' . $_POST['reply_comment_id']);
  exit();
}
...
if (isset($_GET['delete_reply_id'])) {
  ...
  $get_comment_id_stmt = $db->prepare('SELECT comment_id FROM reply_comments WHERE id = ?');
  ...
  if ($comment_id_result !== false) {
    ...
    // Redirect back to the current page with the comment_id parameter
    header('Location: reply_comments.php?comment_id=' . $comment_id);
    exit();
  } else {
...
if ($comment_id !== null) {
  ...
  $replies = $db->prepare('SELECT rc.*, u.artist, u.id as userid FROM reply_comments rc JOIN users u ON rc.email = u.email WHERE rc.comment_id = ? ORDER BY rc.id DESC');
    ...
    <div class="container-fluid mt-2">
      <?php if ($comment_id !== null && $comment !== false): ?>
        ...
        <div class="modal-dialog mt-5 mb-5" role="document">
          <div class="modal-content rounded-3 shadow border-4 border">
            <div class="modal-body p-4 text-center">
              <h5 class="mb-0 fw-bold">Comment Replies</h5>
              <p class="mb-0 fw-bold text-muted mt-3"><?= $comment['comment'] ?></p>
            </div>
          </div>
        </div>
        <?php
        ...
        <div class="card container-fluid mb-2">
          <p class="text-dark fw-bold mt-2"><i class="bi bi-person-circle"></i> <a class="text-dark text-decoration-none" href="artist.php?id=<?php echo $reply['userid'];?>">@<?php echo $reply['artist']; ?></a></p>
          <div class="text-secondary fw-bold mb-2" style="word-break: break-word;" data-lazyload><small><?php echo preg_replace('/\b(https?:\/\/\S+)/i', '<a class="text-decoration-none" target="_blank" href="$1">$1</a>', $reply['reply']); ?></small></div>
          <?php if ($_SESSION['email'] === $reply['email']): ?>
            <form action="" method="get">
              <input type="hidden" name="delete_reply_id" value="<?= $reply['id'] ?>">
              <button onclick="return confirm('Are you sure?')" class="btn btn-sm btn-secondary mb-2" type="submit"><i class="bi bi-trash-fill"></i></button>
            </form>
          <?php endif; ?>
        </div>
        <?php endwhile; ?>
          ...
          <div>
            <form class="form-control border-0" action="" method="POST">
              ...
              <textarea type="text" class="form-control fw-bold rounded-3 mb-2" style="height: 200px; max-height: 800px;" name="reply" id="reply" placeholder="Type something..." aria-label="Type a message..." aria-describedby="basic-addon2" 
                ...
                onkeyup="this.style.height = '40px'; var newHeight = (this.scrollHeight + 10 * (this.value.split(/\r?\n/).length - 1)) + 'px'; if (parseInt(newHeight) > 800) { this.style.height = '800px'; } else { this.style.height = newHeight; }"></textarea>
