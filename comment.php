...
$stmt = $db->prepare("SELECT COUNT(*) FROM comments WHERE filename=:filename");
$stmt->bindValue(':filename', $filename, SQLITE3_TEXT);
$total_comments = $stmt->execute()->fetchArray()[0];

// Calculate the total number of pages
$total_pages = ceil($total_comments / $comments_per_page);

// Get all comments for the current image for the current page
$stmt = $db->prepare("SELECT comments.*, users.artist FROM comments JOIN users ON comments.email = users.email WHERE comments.filename=:filename ORDER BY comments.id DESC LIMIT :comments_per_page OFFSET :offset");
$stmt->bindValue(':filename', $filename, SQLITE3_TEXT);
    ...
    <div class="container-fluid mt-2">
      <div class="row">
        <div class="col-md-8 mb-5 mx-auto">
          ...
          <div class="card mb-2">
            <div class="me-1 ms-1 mt-1 text-secondary fw-bold">
              ...
              <div style="word-break: break-word;" data-lazyload><p><?php echo preg_replace('/\b(https?:\/\/\S+)/i', '<a class="text-decoration-none" target="_blank" href="$1">$1</a>', $comment['comment']); ?></p></div>
              ...
              <?php if ($comment['email'] == $_SESSION['email']) : ?>
                <form action="" method="POST">
                  ...
                  <div class="form-group mb-1 <?php echo $showTextarea ? '' : 'd-none'; ?>">
                    <textarea class="form-control" oninput="stripHtmlTags(this)" name="new_comment" rows="3"><?php echo strip_tags($comment['comment']); ?></textarea>
        ...
        <div class="pagination justify-content-center" style="margin-bottom: 80px;">
          <?php if ($page > 1): ?>
            <a class="btn btn-sm fw-bold btn-primary me-1" href="?filename=<?php echo $filename; ?>&page=<?php echo $page - 1 ?>">Next</a>
          <?php endif ?>
          <?php if ($page < $total_pages): ?>
            <a class="btn btn-sm fw-bold btn-primary ms-1" href="?filename=<?php echo $filename; ?>&page=<?php echo $page + 1 ?>">Prev</a>
          <?php endif ?>
        </div>
