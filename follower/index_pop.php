...
// Get the details of the following users, ordered by the number of followers (most to least)
$stmt = $db->prepare("
    SELECT u.*, SUBSTR(u.artist, 1, 1) AS first_letter, COUNT(f.id) AS follower_count
    FROM users u
    LEFT JOIN following f ON u.email = f.follower_email
    WHERE u.email IN ('" . implode("','", $follower_users) . "')
    GROUP BY u.id
