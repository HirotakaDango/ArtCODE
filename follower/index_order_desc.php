...
$by = $_GET['by'] ?? 'descending'; // Assuming 'descending' is used for Z-A order
$limit = 20;
$offset = ($page - 1) * $limit;
...
// Get the details of the following users, ordered alphabetically by artist name (Z-A)
$stmt = $db->prepare("SELECT *, SUBSTR(artist, 1, 1) AS first_letter FROM users WHERE email IN ('" . implode("','", $follower_users) . "') ORDER BY artist DESC");
$result = $stmt->execute();
