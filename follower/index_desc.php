...
// Get the details of the following users, sorted by newest (id DESC)
$stmt = $db->prepare("SELECT *, SUBSTR(artist, 1, 1) AS first_letter FROM users WHERE email IN ('" . implode("','", $follower_users) . "') ORDER BY id DESC");
$result = $stmt->execute();
