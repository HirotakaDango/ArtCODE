...
<html>
  ...
  <body>
    ...
    <div class="container">
      <h4 class="text-secondary text-center fw-bold">All Creators</h4>
      <?php
      ...
      <div class="tag-buttons" role="group" aria-label="User buttons">
        <?php while ($user = $users->fetchArray()): ?>
          <button type="button" class="btn btn-secondary tag-button" onclick="location.href='artist.php?id=<?= $user['id'] ?>'"><?= $user['artist'] ?></button>
        <?php endwhile; ?>
      </div>
