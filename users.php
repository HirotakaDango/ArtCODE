  ...
  <body>
    ...
    <div class="container-fluid">
      <?php
        ...
        $users = $db->query('SELECT * FROM users ORDER BY artist COLLATE NOCASE ASC');
        ...
        $usersPage = $db->query("SELECT * FROM users ORDER BY artist COLLATE NOCASE ASC LIMIT $perPage OFFSET $offset");
      ?>
        <!-- Display each user's id as a button in a button group -->
        <div class="row">
          <?php while ($user = $usersPage->fetchArray()): ?>
            <div class="col-md-3 col-sm-6">
              <button type="button" class="btn tag-button btn-secondary mb-2 fw-bold text-start w-100 opacity-75" onclick="location.href='artist.php?id=<?= $user['id'] ?>'"><?= $user['artist'] ?></button>
            </div>
          <?php endwhile; ?>
        </div> 
