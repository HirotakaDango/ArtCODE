...
$stmt = $db->prepare("SELECT forum.*, users.artist, users.id as iduser FROM forum JOIN users ON forum.email = users.email ORDER BY forum.id DESC LIMIT :items_per_page OFFSET :offset");
      ...
      ?>
        <div class="card mb-2">
          <div class="me-1 ms-1 mt-1 text-secondary fw-bold">
            <p class="text-dark"><i class="bi bi-person-circle"></i> <a class="text-dark text-decoration-none" href="artist.php?id=<?php echo $comment['iduser'];?>">@<?php echo $comment['artist']; ?></a></p>
            ...
            <a class="btn btn-sm fw-bold" href="reply_forum.php?comment_id=<?php echo $comment['id']; ?>"><i class="bi bi-reply-fill"></i> reply</a></br>
