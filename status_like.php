<?php
// Query to get statuses from users that the current user is following and liked by the current user
$status_query = $db->prepare("
  SELECT users.email, users.artist, users.pic, users.id AS userid, status.message, status.date, status.id, 
         COUNT(favorites_status.id) AS like_count
  FROM users
  JOIN status ON users.email = status.email
  JOIN favorites_status ON status.id = favorites_status.status_id
  WHERE users.email IN (" . implode(',', array_fill(0, count($following_emails), '?')) . ")
    AND favorites_status.email = :current_user_email
  GROUP BY status.id
  HAVING like_count > 0
