...
$status_query = $db->prepare("SELECT users.email, users.artist, users.pic, users.id AS userid, status.message, status.date, status.id, COUNT(favorites_status.id) AS like_count FROM users JOIN status ON users.email = status.email LEFT JOIN favorites_status ON status.id = favorites_status.status_id AND favorites_status.email = :current_user_email WHERE users.email IN (" . implode(',', array_fill(0, count($following_emails), '?')) . ") AND favorites_status.id IS NOT NULL GROUP BY status.id ORDER BY status.id DESC");
