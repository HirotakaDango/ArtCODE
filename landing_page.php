  <?php include('lp_header.php'); ?>
  <div class="container-fluid big-gap">
    <div class="row featurette mt-4">
      <div class="container text-center col-md-7 order-md-2">
        <h1 class="animate__animated animate__fadeInUp animate__delay-0-5s display-5 fw-bold">Welcome to ArtCODE</h1>
        <p class="animate__animated animate__fadeInUp animate__delay-0-5s text-secondary fs-5 fw-bold">Immerse yourself in a tapestry of colors, shapes, and emotions as you explore the vast gallery of artistic expression. From awe-inspiring paintings to mesmerizing digital art, our collection embraces diversity and showcases the ingenuity of artists from around the globe.</p>
        <p class="animate__animated animate__fadeInUp animate__delay-0-5s text-secondary fs-5 fw-bold">Discover, create, and connect through art</p>
        <header class="d-flex justify-content-center py-3">
          <ul class="nav nav-pills">
            <li class="nav-item"><a href="#" class="btn fw-bold btn-primary me-2 animate__animated animate__fadeInUp animate__delay-0-5s" data-bs-toggle="modal" data-bs-target="#signin">Signin</a></li>
            <li class="nav-item"><a href="#" class="btn fw-bold btn-outline-primary animate__animated animate__fadeInUp animate__delay-0-5s" data-bs-toggle="modal" data-bs-target="#signup">Signup</a></li>
          </ul>
        </header>
      </div>
      <div class="col-md-5 order-md-1 animate__animated animate__fadeInUp animate__delay-0-5s">
        <img class="img-fluid" src="icon/Bloging.svg">
      </div>
    </div>
  </div>
  <div class="container fw-bold">
    <div class="row featurette">
      <div class="col-md-7">
        <h1 class="animate__animated animate__fadeInDown animate__delay-1s display-5 fw-bold">Discover amazing artworks</h1>
        <p class="animate__animated animate__fadeInDown animate__delay-1s text-secondary fs-5">Explore our vast collection of beautiful artworks, created by talented artists from all over the world.</p>
        <p class="animate__animated animate__fadeInDown animate__delay-1s text-secondary fs-5">From digital art to traditional paintings, ArtCODE has it all.</p>
        <a href="preview_guest.php" class="btn fw-bold btn-primary rounded-pill animate__animated animate__fadeInDown animate__delay-1s">Start Exploring</a>
      </div>
      <div class="col-md-5 animate__animated animate__fadeInUp animate__delay-1s">
        <img class="img-fluid" src="icon/Global logistics delivery network.svg">
      </div>
    </div>
    <div class="row featurette mt-4">
      <div class="col-md-7 order-md-2">
        <h1 class="animate__animated animate__fadeInUp animate__delay-2s display-5 fw-bold">Share your creativity</h1>
        <p class="animate__animated animate__fadeInUp animate__delay-2s text-secondary fs-5">Join our community and showcase your artwork to the world.</p>
        <p class="animate__animated animate__fadeInUp animate__delay-2s text-secondary fs-5">Connect with other artists, get feedback, and discover new opportunities.</p>
        <a href="#" class="btn fw-bold btn-primary rounded-pill animate__animated animate__fadeInUp animate__delay-2s" data-bs-toggle="modal" data-bs-target="#signup">Join Now</a>
      </div>
      <div class="col-md-5 order-md-1 animate__animated animate__fadeInUp animate__delay-2s">
        <img class="img-fluid" src="icon/Writing room.svg">
      </div>
    </div>
  </div>
  <?php include('about.php'); ?>
  <?php include('footer.php'); ?>