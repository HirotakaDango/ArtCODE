  ...
  <head>
    ...
    <style>
      ...
      #noteContent {
        ...
        overflow-y: auto;
        box-sizing: border-box;
        /* REAL MAGIC: fill the viewport below sticky header, no double scroll, bottom always visible */
  ...
  <body>
    <div class="w-100">
      ...
      <div class="container mb-5 mt-3" id="allNotesView">
        ...
        <ul class="nav nav-tabs border-0 my-2 bg-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?>-subtle p-3 rounded-4 align-items-center">
          <li class="nav-item">
            <input id="searchInput" class="form-control search-sm rounded search-input w-100 border-0" type="text" placeholder="🔍 Search..." />
          </li>
      ...
      <div id="singleNoteView" style="display: none;">
        <div id="singleNoteDualCol" class="d-flex flex-row">
          ...
          <div id="singleNoteSidebarColumn" class="d-flex flex-column vh-100 overflow-auto p-3 gap-1 bg-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?>-subtle" style="display: flex;min-width:350px;max-width:420px;">
          ...
          <div id="singleNoteMainColumn" class="flex-grow-1 d-flex flex-column">
            <div id="singleNoteMainNavbar" class="d-flex justify-content-between align-items-center bg-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?>-subtle py-1 px-auto">
            ...
            <!-- The textarea now fills the rest, scrolls inside the column, navbar is sticky, dvh fix -->
            <textarea id="noteContent"
              class="form-control rounded-0 p-4 border-0 bg-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?>-subtle focus-ring text-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> focus-ring-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?> flex-grow-1"></textarea>
          </div>
        </div>
      </div>
    </div>
    <input type="file" id="importInput" style="display: none;" accept=".json" />
    <div id="wordCounter" class="bg-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> text-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?> bg-opacity-50" style="display: none;">Words: 0</div>

    <button id="installButton" class="btn btn-primary shadow-lg d-none">
    ...
    <script>
      ...
      function renderNoteCard(note, noteId, {showCheckbox=false, selected=false, showCategory=true, showActions=true, active=false, limitChars=200}={}) {
        ...
        // Category label (always below date + actions row)
        let categoryRow = document.createElement('div');
        categoryRow.className = "mt-1";
        if (showCategory) {
          let categoryElem = document.createElement('span');
          categoryElem.className = "category-badge text-truncate";
          categoryElem.title = note.category || "all";
          categoryElem.textContent = note.category || "all";
          categoryRow.appendChild(categoryElem);
        }
        ...
        cardBody.append(noteHeader, categoryRow, contentDiv);
      ...
      function registerServiceWorkerInline() {
        if ('serviceWorker' in navigator) {
          const swCode = `
            ...
            const urlsToCache = [
              (location.pathname.endsWith('/') ? location.pathname + 'index.html' : location.pathname.endsWith('.html') ? location.pathname : location.pathname + '/index.html'),
            ...
            self.addEventListener('fetch', e => {
              e.respondWith(
                fetch(e.request).then(r=>r).catch(() => caches.match(e.request).then(r=>r||caches.match((location.pathname.endsWith('/') ? location.pathname + 'index.html' : location.pathname.endsWith('.html') ? location.pathname : location.pathname + '/index.html'))))
              );
            });
          `;
          try {
