...
$query = $db->prepare('SELECT images.id, images.tags, images.filename, images.title, images.imgdesc, images.type, images.view_count FROM images JOIN users ON images.email = users.email WHERE users.id = :id ORDER BY images.view_count DESC LIMIT :limit OFFSET :offset');
$query->bindParam(':id', $id);
...
?>
    ...
    <div class="pagination d-flex gap-1 justify-content-center mt-3">
      <?php if ($page > 1): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?id=<?php echo $id; ?>&by=view&page=1"><i class="bi text-stroke bi-chevron-double-left"></i></a>
        <a class="btn btn-sm btn-primary fw-bold" href="?id=<?php echo $id; ?>&by=view&page=<?php echo $prevPage; ?>"><i class="bi text-stroke bi-chevron-left"></i></a>
      <?php endif; ?>
      ...
      <?php
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
          if ($i === $page) {
            ...
            echo '<a class="btn btn-sm btn-primary fw-bold" href="?id=' . $id . '&by=view&page=' . $i . '">' . $i . '</a>';
          }
        }
      ?>

      <?php if ($page < $totalPages): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?id=<?php echo $id; ?>&by=view&page=<?php echo $nextPage; ?>"><i class="bi text-stroke bi-chevron-right"></i></a>
        <a class="btn btn-sm btn-primary fw-bold" href="?id=<?php echo $id; ?>&by=view&page=<?php echo $totalPages; ?>"><i class="bi text-stroke bi-chevron-double-right"></i></a>
      <?php endif; ?>
    </div>
    <div class="mt-5"></div>