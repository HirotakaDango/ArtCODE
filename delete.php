...
try {
  ...
  $stmt = $db->prepare("DELETE FROM reply_comments WHERE comment_id IN (SELECT id FROM comments WHERE filename = $id)");
  $stmt->bindValue(':filename', $id);
  $stmt->execute();
...
header("Location: profile.php?by=$byParam&page=$pageParam");
exit;
?>
