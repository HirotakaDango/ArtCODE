...
// Get characters for the current category and sort them by view_count of images
$currentcharacters = isset($groupedcharacters[$category]) ? $groupedcharacters[$category] : [];

// Fetch view_count for each character
$viewCounts = [];
foreach ($currentcharacters as $character => $count) {
  $stmt = $db->prepare("SELECT SUM(view_count) as total_views FROM images WHERE characters LIKE ?");
  $stmt->bindValue(1, '%' . $character . '%');
  $result = $stmt->execute()->fetchArray(SQLITE3_ASSOC);
  $viewCounts[$character] = $result['total_views'] ?? 0;
}

// Sort characters by view_count in ascending order (least viewed first)
array_multisort($viewCounts, SORT_ASC, $currentcharacters);
