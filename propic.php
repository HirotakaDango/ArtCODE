...
$stmt = $db->prepare('SELECT pic FROM users WHERE username = :username');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($_FILES['pic']['name'])) {
    ...
    if (move_uploaded_file($temp_file, $file_path)) {
      ...
      $db = new SQLite3('database.sqlite');
      $stmt = $db->prepare('UPDATE users SET pic = :pic WHERE username = :username');
...
?>
    ...
    <?php endif; ?>
    <div class="mt-3">
    <h5 class="card-title text-center text-secondary fw-bold mb-3">Add Profile Picture</h5>
      <div class="roow">
        <div class="cool-6">
          <div class="caard">
          ...
          <div class="caard container">
            <form method="post" enctype="multipart/form-data">
              <div class="form-group mb-3">
                <label for="pic" class="form-label text-secondary fw-bold">Select a profile picture:</label>
                <input type="file" id="pic" name="pic" class="form-control rounded-3 border text-secondary fw-bold border-4">
              </div>
              <header class="d-flex justify-content-center">
                <ul class="nav nav-pills">
                  <li class="nav-item"><button type="submit" class="btn btn-w btn-primary fw-bold">Apply</button></li>
                  <li class="nav-item d-md-none d-lg-none"><a href="setting.php" class="btn btn-w btn-danger ms-1 fw-bold">Back</a></li>
                </ul>
              </header>
            </form>
          </div> 
