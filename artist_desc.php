...
$query = $db->prepare('SELECT images.id, images.tags, images.filename, images.title, images.imgdesc, images.type FROM images JOIN users ON images.email = users.email WHERE users.id = :id ORDER BY images.id DESC LIMIT :limit OFFSET :offset');
$query->bindParam(':id', $id);
