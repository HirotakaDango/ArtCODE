...
<html lang="en" data-bs-theme="<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?>">
  <head>
    ...
    <title>Chat with <?php echo htmlspecialchars($chat_user['artist']); ?></title>
  ...
  <body>
    <div class="fixed-top container rounded-bottom-4 bg-dark-subtle">
      <div class="d-flex justify-content-center align-items-center position-relative py-4">
        <a class="btn border-0 position-absolute top-50 start-0 translate-middle ms-2" href="/messages/"><i class="bi bi-chevron-left" style="-webkit-text-stroke: 2px;"></i></a>
        <h5 class="position-absolute top-50 start-50 translate-middle pb-1">Chat with <?php echo $chat_user['artist']; ?></h5>
        <div class="dropdown position-absolute top-50 start-100 translate-middle pe-4">
          <button class="btn border-0" type="button" data-bs-toggle="dropdown" aria-expanded="false">
            <i class="bi bi-three-dots-vertical"></i>
          </button>
          <ul class="dropdown-menu">
            <li><button class="dropdown-item" id="toggleButton" onclick="toggleAutoScroll()">Auto-Scroll: ON</button></li>
          </ul>
        </div>
      </div>
    </div>
    ...
    <div class="fixed-bottom container py-3">
      <form id="messageForm">
        <div class="input-group w-100 rounded-0 shadow-lg rounded-4">
          ...
          <textarea id="message" name="message" class="form-control fw-medium bg-body-tertiary border-0 rounded-start-5 focus-ring focus-ring-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?>" style="height: 40px; max-height: 150px;" placeholder="Type a message..." aria-label="Type a message..." aria-describedby="basic-addon2" 
