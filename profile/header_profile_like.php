    <div class="overflow-x-auto container-fluid p-1 mb-2 hide-scrollbar" style="white-space: nowrap; overflow: auto;">
      <a href="?by=liked&page=<?= isset($_GET['page']) ? htmlspecialchars($_GET['page']) : '1'; ?>" class="fw-medium btn btn-sm btn-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> rounded-pill">all images</a>
      <?php
        try {
      ...
      <?php if (!empty($tags)): ?>
        <?php foreach ($tags as $row): ?>
          ...
          ?>
          <a class="fw-medium btn btn-sm btn-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> rounded-pill" style="margin-right: 2px;" href="?by=tagged_liked&tag=<?= $firstTag ?>">
