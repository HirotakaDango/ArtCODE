    <div class="w-100 px-1">
      <div class="<?php include('../rows_columns/row-cols.php'); echo $rows_columns; ?>">
        <?php foreach ($results as $imageW): ?>
          <div class="col">
            <div class="position-relative">
              <a class="rounded ratio ratio-1x1" href="../image.php?artworkid=<?php echo $imageW['id']; ?>">
                <img class="rounded rounded-bottom-0 shadow object-fit-cover lazy-load <?php echo ($imageW['type'] === 'nsfw') ? 'nsfw' : ''; ?>" data-src="../thumbnails/<?php echo $imageW['filename']; ?>" alt="<?php echo $imageW['title']; ?>">
              </a> 
              <?php
                $current_image_id = $imageW['id'];
              ...
              <?php include('../rows_columns/image_counts.php'); ?>
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li><button class="dropdown-item fw-bold" onclick="location.href='../edit_image.php?id=<?php echo $imageW['id']; ?>'" ><i class="bi bi-pencil-fill"></i> edit image</button></li>
                    <li><button class="dropdown-item fw-bold" data-bs-toggle="modal" data-bs-target="#deleteImage_<?php echo $imageW['id']; ?>"><i class="bi bi-trash-fill"></i> delete</button></li>
                    <?php
                    ...
                    $stmt->bindValue(':image_id', $imageW['id']);
                    $stmt->execute();
                    ...
                    ?>
                    <form method="POST">
                      <input type="hidden" name="image_id" value="<?php echo $imageW['id']; ?>">
                      <li>
                    ...
                    <li><button class="dropdown-item fw-bold" data-bs-toggle="modal" data-bs-target="#shareImage<?php echo $imageW['id']; ?>"><i class="bi bi-share-fill"></i> <small>share</small></button></li>
                    <li><button class="dropdown-item fw-bold" data-bs-toggle="modal" data-bs-target="#infoImage_<?php echo $imageW['id']; ?>"><i class="bi bi-info-circle-fill"></i> <small>info</small></button></li>
                  </ul>
                  <?php include('share_profile_week.php'); ?>
                </div>
              </div>
            </div>

            <?php include($_SERVER['DOCUMENT_ROOT'] . '/profile/components/delete_image_week.php'); ?>
            <?php include($_SERVER['DOCUMENT_ROOT'] . '/profile/components/card_image_week.php'); ?>
