...
$stmt->execute();
$stmt = $db->prepare('CREATE TABLE IF NOT EXISTS status ( id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, message TEXT, date DATETIME)');
$stmt->execute();
...
if (isset($_POST['favorite'])) {
  ...
  if (isset($_GET['by']) && $_GET['by'] == 'oldest') {
    header('Location: ' . $_SERVER['PHP_SELF'] . '?by=oldest&page=' . $_GET['page']);
  } else {
    header('Location: ' . $_SERVER['PHP_SELF'] . '?by=newest&page=' . $_GET['page']);
  }
  exit();
