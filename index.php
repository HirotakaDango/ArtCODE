...
if (!isset($_SESSION['email'])) {
  ...
  // The user is logged in, check if a token exists and start the session
  ...
  if ($result && isset($result['token'])) {
    if ($_SESSION['token'] !== $result['token']) {
      // Update the session token to match the latest token from the database
      $_SESSION['token'] = $result['token'];
    }
  } else {
    // Redirect to login if token doesn't exist (user needs to log in manually)
    header("Location: session.php");
    exit;
  }
}
