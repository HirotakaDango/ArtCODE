...
if (isset($_GET['reply_id'])) {
  ...
  $stmt = $db->prepare('SELECT * FROM reply_forum WHERE id = :reply_id');
  ...
  if ($reply && $reply['email'] === $_SESSION['email']) {
    ...
    if (isset($_POST['reply'])) {
      ...
      if (!empty($newReply)) {
        ...
        $stmt = $db->prepare('UPDATE reply_forum SET reply = :new_reply WHERE id = :reply_id');
...
<html lang="en" data-bs-theme="<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/mode.php'); ?>">
  ...
  <body>
    ...
    <script>
      function goBack() {
        window.location.href = "reply_forum.php?comment_id=<?php echo urlencode($reply['comment_id']); ?>";
      }
    </script>
