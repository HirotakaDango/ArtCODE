...
$stmtP = $dbP->prepare("SELECT images.id, images.filename, images.tags, images.title, images.imgdesc, COUNT(favorites.id) AS favorite_count FROM images LEFT JOIN favorites ON images.id = favorites.image_id GROUP BY images.id ORDER BY favorite_count DESC LIMIT 70");
