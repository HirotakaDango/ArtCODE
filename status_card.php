        <div class="card border-0 shadow mb-1 position-relative p-2 bg-body-tertiary rounded-4">
          <div class="d-flex align-items-center mb-2 position-relative">
            <div class="position-absolute top-0 start-0 m-1">
              ...
              <a class="text-decoration-none fw-medium link-body-emphasis" href="artist.php?id=<?php echo $message['userid'];?>" target="_blank"><small>@<?php echo (mb_strlen($message['artist']) > 15) ? mb_substr($message['artist'], 0, 15) . '...' : $message['artist']; ?></small></a>・<small class="small fw-medium"><small><?php echo date('Y/m/d', strtotime($message['date'])); ?></small></small>
            </div>
          </div>
          <div class="mt-5 container-fluid">
            ...
            <?php if ($message['email'] == $email): ?>
              <form method="post" action="">
                ...
                <button type="submit" name="delete" class="btn btn-outline-dark border-0 btn-sm position-absolute top-0 end-0 m-2 text-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?>" onclick="return confirm('Are you sure?')"><i class="bi bi-trash"></i></button>
              </form>
            <?php endif; ?>
            <div class="mt-5">
              <?php
