...
$whereClauses = [];
$bindings = [];
$selectClause = "SELECT private_images.*";
...
$totalQueryString = "SELECT COUNT(private_images.id) " . $fromClause . $joinClause . $sqlWhere;
...
if ($stmt === false) {
  ...
  // Bind monthly popularity range and limit/offset
  $stmt->bindValue(':startDate', $startOfMonth, SQLITE3_TEXT);
  $stmt->bindValue(':endDate', $endOfMonth, SQLITE3_TEXT);
