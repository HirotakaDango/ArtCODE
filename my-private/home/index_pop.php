...
$whereClauses = [];
$bindings = [];
$selectClause = "SELECT private_images.*, COUNT(favorites.id) AS favorite_count";
$fromClause = "FROM private_images";
$joinClause = " LEFT JOIN private_favorites ON private_images.id = private_favorites.image_id";

$needsUsersJoin = false;

$whereClauses[] = "private_images.email = :email";
...
if (!empty($_GET['q'])) {
  ...
  if (!empty($q_input_terms)) {
    ...
    // Added artist and title fields to search as requested
    $q_search_fields = ["users.artist", "private_images.title", "private_images.characters", "private_images.\"group\"", "private_images.categories", "private_images.tags", "private_images.parodies", "private_images.language"];
    $needsUsersJoin = true;
...
if (!empty($_GET['uid']) && filter_var($_GET['uid'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1]])) {
  // If not yet in join, add users join
  $needsUsersJoin = true;
  $whereClauses[] = "users.id = :filter_uid";
  $bindings[':filter_uid'] = (int)$_GET['uid'];
}

// Add users join if needed
if ($needsUsersJoin) {
  if (strpos($joinClause, 'users') === false) {
    $joinClause .= " INNER JOIN users ON private_images.email = users.email";
  }
}

// Date filters (added from index_desc.php)

// Helper function to add date IN filter
...
// Total count query - do not count DISTINCT private_images.id if grouping, use COUNT(DISTINCT private_images.id) for accurate pagination
$totalQueryString = "SELECT COUNT(DISTINCT private_images.id) " . $fromClause . $joinClause . $sqlWhere;
...
// Main query: use GROUP BY and ORDER BY favorite_count DESC
$imagesQueryString = $selectClause . " " . $fromClause . $joinClause . $sqlWhere . 
                     " GROUP BY private_images.id ORDER BY favorite_count DESC LIMIT :page_limit OFFSET :page_offset";
