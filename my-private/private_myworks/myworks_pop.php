...
$stmt = $db->prepare("SELECT private_images.*, COUNT(private_favorites.id) AS favorite_count FROM private_images LEFT JOIN private_favorites ON private_images.id = private_favorites.image_id WHERE private_images.email = :email GROUP BY private_images.id ORDER BY favorite_count DESC LIMIT :limit OFFSET :offset");
