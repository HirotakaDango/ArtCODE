...
if (isset($_GET['tag'])) {
  ...
  $stmt = $db->prepare("SELECT * FROM private_images WHERE email = :email AND tags LIKE :tagPattern ORDER BY title ASC LIMIT :limit OFFSET :offset");
  ...
  $stmt = $db->prepare("SELECT * FROM private_images WHERE email = :email ORDER BY title ASC LIMIT :limit OFFSET :offset");
  $stmt->bindValue(':email', $email, SQLITE3_TEXT);
  $stmt->bindValue(':limit', $limit, SQLITE3_INTEGER);
  $stmt->bindValue(':offset', $offset, SQLITE3_INTEGER);
}
