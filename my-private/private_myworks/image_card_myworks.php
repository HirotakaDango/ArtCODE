        ...
        <?php while ($image = $result->fetchArray()): ?>
          <div class="col">
            <div class="position-relative">
              <a class="rounded ratio ratio-1x1" href="/private_image.php?artworkid=<?php echo $image['id']; ?>">
                <img class="rounded rounded-bottom-0 shadow object-fit-cover lazy-load <?php echo ($image['type'] === 'nsfw') ? 'nsfw' : ''; ?>" data-src="/private_thumbnails/<?php echo $image['filename']; ?>" alt="<?php echo $image['title']; ?>">
              </a> 
              <?php
                $current_image_id = $image['id'];
                
                // Query to count main image from the private_images table
                $stmt = $db->prepare("SELECT COUNT(*) as image_count FROM private_images WHERE id = :id");
                ...
                // Query to count associated private_images from the private_image_child table
                $stmt = $db->prepare("SELECT COUNT(*) as child_image_count FROM private_image_child WHERE image_id = :image_id");
                ...
                // Total count of main private_images and associated private_images
              ...
              <?php include('../../rows_columns/image_counts.php'); ?>
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li><button class="dropdown-item fw-bold" onclick="location.href='edit_image.php?id=<?php echo $image['id']; ?>&back=<?php echo urlencode('http' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']); ?>'"><i class="bi bi-pencil-fill"></i> edit image</button></li>
                    ...
                    <?php
                      $is_favorited = $db->querySingle("SELECT COUNT(*) FROM private_favorites WHERE email = '$email' AND image_id = {$image['id']}");
                      if ($is_favorited) {
                    ?>
                      <form method="POST">
            ...
            <?php include('delete_image_myworks.php'); ?>
            <?php include('card_image_myworks.php'); ?>
