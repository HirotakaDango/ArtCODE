<?php
// Get all private_images for the given user_email
$stmt = $db->prepare("SELECT id, filename, tags, title, imgdesc, type FROM private_images WHERE email = :email ORDER BY id DESC");
...
?>
                            ...
                            <div class="col">
                              <a href="?artworkid=<?php echo $image_id; ?>&mode=<?php echo $_GET['mode']; ?>" class="position-relative">
                                <div class="ratio ratio-1x1">
                                  <img class="object-fit-cover rounded <?php echo ($imageU['type'] === 'nsfw') ? 'blurred' : ''; ?> <?php echo ($image_id == $current_image_id) ? 'opacity-50' : ''; ?>" src="/private_thumbnails/<?php echo $image_url; ?>" alt="<?php echo htmlspecialchars($image_title); ?>" style="object-fit: cover;">
                                </div>
                                <?php
                                // Example of error handling and querying
                                try {
                                  $stmt_count_main = $db->prepare("SELECT COUNT(*) as image_count FROM private_images WHERE id = :id");
              ...
              <div class="container px-0 d-md-none">
                <div id="imageCarouselUserMobile" class="carousel slide" data-bs-ride="carousel">
                  ...
                  <div class="d-flex mt-2">
                    <button class="me-auto btn btn-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> p-1 py-0" type="button" data-bs-target="#imageCarouselUserMobile" data-bs-slide="prev">
                      <i class="bi bi-chevron-left" style="-webkit-text-stroke: 1px;"></i>
                    </button>
                    <button class="ms-auto btn btn-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> p-1 py-0" type="button" data-bs-target="#imageCarouselUserMobile" data-bs-slide="next">
                      <i class="bi bi-chevron-right" style="-webkit-text-stroke: 1px;"></i>
                    </button>
                  </div>
                </div>
