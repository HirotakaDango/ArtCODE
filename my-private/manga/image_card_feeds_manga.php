              ...
              <div class="card border-0 rounded-4">
                <a href="title.php?title=<?= urlencode($image['episode_name']); ?>&uid=<?= $image['userid']; ?>" class="text-decoration-none">
                  <div class="ratio ratio-cover">
                    <img class="rounded rounded-bottom-0 object-fit-cover lazy-load" data-src="/private_thumbnails/<?= $image['filename']; ?>" alt="<?= $image['title']; ?>">
                  </div>
                  <h6 class="text-center fw-bold text-<?php include($_SERVER['DOCUMENT_ROOT'] . '/appearance/opposite.php'); ?> text-decoration-none bg-dark-subtle p-2 rounded rounded-top-0" id="episode-name_img<?= $image['id']; ?>_<?= $image['userid']; ?>">「<?= $ownerArtist; ?>」<?= $image['episode_name']; ?>「<?= $image['language']; ?>」</h6>
                </a>
              </div>
            </div>
            <style>
