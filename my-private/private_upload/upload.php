...
// Check if any private_images were uploaded
if (isset($_FILES['image'])) {
  ...
  $uploadDir = '../../private_images/';
  $thumbnailDir = '../../private_thumbnails/';
  ...
  $stmt = $db->prepare("UPDATE private_images SET filename = :filename WHERE id = :id");
  ...
  // Process additional (child) private_images if any
  for ($i = 1; $i < count($images['name']); $i++) {
    ...
    if ($image['error'] === 0) {
      ...
      $stmt = $db->prepare("INSERT INTO private_image_child (filename, original_filename, image_id, email) VALUES (:filename, :original_filename, :image_id, :email)");
