...
// Check if the current session user is the owner (matches $userId)
$ownerCheckStmt = $db->prepare('SELECT id FROM users WHERE email = :email');
$ownerCheckStmt->bindValue(':email', $email, SQLITE3_TEXT);
$ownerCheckResult = $ownerCheckStmt->execute();
$ownerRow = $ownerCheckResult->fetchArray(SQLITE3_ASSOC);

if (!$ownerRow || $ownerRow['id'] != $userId) {
  // Not the owner, redirect to parent directory
  header("Location: ../");
  exit();
}
...
if (isset($_POST['favorite'])) {
  ...
  $existingFavStmt = $db->prepare("SELECT COUNT(*) as count FROM private_favorites WHERE email = :email AND image_id = :image_id");
  $existingFavStmt->bindValue(':email', $email, SQLITE3_TEXT);
  $existingFavStmt->bindValue(':image_id', $image_id, SQLITE3_INTEGER);
  $existingFavResult = $existingFavStmt->execute();
  $existing_fav = $existingFavResult->fetchArray(SQLITE3_ASSOC)['count'];

  if ($existing_fav == 0) {
    $insertFavStmt = $db->prepare("INSERT INTO private_favorites (email, image_id) VALUES (:email, :image_id)");
    $insertFavStmt->bindValue(':email', $email, SQLITE3_TEXT);
    $insertFavStmt->bindValue(':image_id', $image_id, SQLITE3_INTEGER);
  ...
  $image_id = $_POST['image_id'];
  $deleteFavStmt = $db->prepare("DELETE FROM private_favorites WHERE email = :email AND image_id = :image_id");
  $deleteFavStmt->bindValue(':email', $email, SQLITE3_TEXT);
  $deleteFavStmt->bindValue(':image_id', $image_id, SQLITE3_INTEGER);
...
// Get the total number of private_images for the specified episode and user ID
$totalStmt = $db->prepare("SELECT COUNT(*) as total FROM private_images WHERE episode_name = :episodeName AND email = :email");
$totalStmt->bindValue(':episodeName', $episodeName, SQLITE3_TEXT);
$totalStmt->bindValue(':email', $email, SQLITE3_TEXT);
$totalResult = $totalStmt->execute();
$total = $totalResult->fetchArray(SQLITE3_ASSOC)['total'];

// Count the total number of private_images for the specified episode and user ID with non-empty episode names
$countStmt = $db->prepare("
  SELECT COUNT(*) as count
  FROM private_images
  WHERE episode_name = :episodeName AND episode_name != '' AND email = :email
");
...
$countViewsStmt = $db->prepare("
  ...
  FROM private_images
  WHERE episode_name = :episodeName AND email = :email
");
$countViewsStmt->bindValue(':episodeName', $episodeName, SQLITE3_TEXT);
$countViewsStmt->bindValue(':email', $email, SQLITE3_TEXT);
$totalViewsResult = $countViewsStmt->execute();
$totalViews = $totalViewsResult->fetchArray(SQLITE3_ASSOC)['total_views'];

// Prepare SQL query to count private_favorites for private_images in the current episode and user ID
...
$countFavoritesStmt->bindValue(':email', $email, SQLITE3_TEXT);
...
$firstImageStmt->bindValue(':email', $email, SQLITE3_TEXT);
      ...
      <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content bg-transparent border-0 rounded-0">
          <div class="modal-body position-relative">
            <img class="object-fit-contain h-100 w-100 rounded" src="/private_images/<?php echo $firstEpisode; ?>">
            ...
            <a class="btn btn-primary fw-bold w-100 mt-2" href="/private_images/<?php echo $firstEpisode; ?>" download>Download Cover Image</a>
          </div>
        </div>
      </div>
    </div>
