...
try {
  ...
  if (isset($_GET['artworkid'])) {
    ...
    // Get the image details from the private_images table for the logged-in user only
    $query = "
      SELECT 
        private_images.*, 
        users.id AS userid, 
      ...
      WHERE private_images.id = :image_id AND private_images.email = :email
    ";
    ...
    // If image does not exist or does not belong to this user, redirect
    if (!$image_details) {
      header("Location: /my-private/~");
      exit;
    }

    // Get all child private_images for the current image ID
    $query_child = "SELECT * FROM private_image_child WHERE image_id = :image_id";
    ...
    // Base URL for private_images. Adjust if needed.
    $web = "/";
  } else {
    // No artworkid param
    header("Location: /my-private/~");
      ...
      <div class="d-flex justify-content-center align-items-center vh-100-sm">
        <div class="w-100">
          <div class="row row-cols-2 row-cols-sm-2 row-cols-md-4 row-cols-lg-6 g-1">
            ...
            <div class="col">
              <a href="/my-private/view/manga/?artworkid=<?php echo $image_id; ?>&page=1" onclick="event.preventDefault(); window.top.location.href=this.href;">
            ...
            <?php
              $page_number = 2; // Child private_images start with page 2
              foreach ($image_child as $child) :
            ?>
              <div class="col">
                <a href="/my-private/view/manga/?artworkid=<?php echo $image_id; ?>&page=<?= $page_number; ?>" onclick="event.preventDefault(); window.top.location.href=this.href;">
                  <div class="ratio-cover">
