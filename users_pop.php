...
// Retrieve users you are following based on the search query
$query = "SELECT users.*, SUBSTR(users.artist, 1, 1) AS first_letter, COUNT(following.follower_email) AS follower_count 
          ...
          WHERE following.follower_email = :follower_email"; // Adjust this to your actual column names
$params = [':follower_email' => $_SESSION['user_email']]; // Assuming you have a session storing user's email

if (!empty($searchQuery)) {
  $query .= " AND artist LIKE :searchQuery";
  $params[':searchQuery'] = '%' . $searchQuery . '%';
}

$query .= " GROUP BY users.email ORDER BY first_letter COLLATE NOCASE DESC, users.artist COLLATE NOCASE ASC";
        ...
        <div class="row justify-content-center">
          <?php foreach ($groupedUsers as $group => $users): ?>
            <div class="col-4 col-md-2 col-sm-5 px-0">
              <a class="btn btn-outline-dark border-0 fw-medium d-flex flex-column align-items-center" href="users.php?by=<?php echo $by; ?>&category=<?php echo $group; ?>">
