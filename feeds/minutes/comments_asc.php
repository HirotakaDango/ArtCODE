...
$stmt = $db->prepare("SELECT comments_minutes.*, users.artist, users.pic, users.id as iduser FROM comments_minutes JOIN users ON comments_minutes.email = users.email WHERE comments_minutes.minute_id=:minute_id ORDER BY comments_minutes.id ASC LIMIT :comments_per_page OFFSET :offset");
