...
$stmt = $db->prepare("SELECT comments_minutes.*, users.artist, users.pic, users.id as iduser FROM comments_minutes JOIN users ON comments_minutes.email = users.email WHERE comments_minutes.minute_id=:minute_id ORDER BY comments_minutes.id DESC LIMIT :comments_per_page OFFSET :offset");
...
?>
      ...
      ?>
        <div class="card border-0 shadow mb-1 position-relative p-2 bg-body-tertiary rounded-4">
          ...
          <div class="m-2 ms-auto">
            <a class="btn btn-sm fw-semibold" href="reply_comment_minute.php?minuteid=<?php echo $minute_id; ?>&comment_id=<?php echo $comment['id']; ?>"><i class="bi bi-reply-fill"></i> Reply</a>
          </div>
        </div>
      <?php
        endwhile;
    ...
    <div class="pagination d-flex gap-1 justify-content-center mt-3">
      <?php if ($page > 1): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=newest&minute_id=<?php echo $minute_id; ?>&page=1"><i class="bi text-stroke bi-chevron-double-left"></i></a>
      <?php endif; ?>

      <?php if ($page > 1): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=newest&minute_id=<?php echo $minute_id; ?>&page=<?php echo $prevPage; ?>"><i class="bi text-stroke bi-chevron-left"></i></a>
      <?php endif; ?>
      ...
      <?php
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
          if ($i === $page) {
            ...
            echo '<a class="btn btn-sm btn-primary fw-bold" href="?by=newest&minute_id=' . $minute_id . '&page=' . $i . '">' . $i . '</a>';
          }
        }
      ?>

      <?php if ($page < $totalPages): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=newest&minute_id=<?php echo $minute_id; ?>&page=<?php echo $nextPage; ?>"><i class="bi text-stroke bi-chevron-right"></i></a>
      <?php endif; ?>

      <?php if ($page < $totalPages): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=newest&minute_id=<?php echo $minute_id; ?>&page=<?php echo $totalPages; ?>"><i class="bi text-stroke bi-chevron-double-right"></i></a>
      <?php endif; ?>
    </div>