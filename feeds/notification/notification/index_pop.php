<?php
// Get images from the database uploaded by users that the current user follows, sorted by popularity
$stmt = $db->prepare("SELECT images.*, COUNT(favorites.id) AS favorite_count FROM images INNER JOIN following ON images.email = following.following_email LEFT JOIN favorites ON images.id = favorites.image_id WHERE following.follower_email = :email GROUP BY images.id ORDER BY favorite_count DESC LIMIT :limit OFFSET :offset");
