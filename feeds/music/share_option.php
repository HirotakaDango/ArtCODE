    <?php
      $currentUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];

      $mode = isset($_GET['mode']) ? ($_GET['mode'] === 'grid' ? (isset($_GET['by']) && ($_GET['by'] === 'oldest' || $_GET['by'] === 'newest') ? '&by=' . $_GET['by'] : '&by=newest') : '') : '&by=newest_lists';

      $album = rawurlencode($row['album']);
      $id = urlencode($row['id']);

      $url = rawurlencode($currentUrl . '?mode=' . $mode . '&album=' . $album . '&id=' . $id);
    ?>
        ...
        <div class="modal-content bg-transparent border-0 rounded-0">
          <div class="card rounded-4 p-4">
            ...
            <div class="btn-group w-100 mb-2" role="group" aria-label="Share Buttons">
              <!-- Twitter -->
              <a class="btn rounded-start-4" href="https://twitter.com/intent/tweet?url=" target="_blank" rel="noopener noreferrer">
            ...
            <div class="btn-group w-100 mb-2" role="group" aria-label="Share Buttons">
              ...
              <!-- Snapchat -->
              <a class="btn rounded-end-4" href="https://www.snapchat.com/share?url=" target="_blank" rel="noopener noreferrer">
