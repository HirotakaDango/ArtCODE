...
try {
  $result = $db->prepare("SELECT novel.*, users.id AS user_id, users.email, users.artist, COUNT(favorites_novel.novel_id) AS favorites_count
                         FROM novel 
                         JOIN users ON novel.email = users.email 
                         LEFT JOIN favorites_novel ON novel.id = favorites_novel.novel_id 
                         WHERE novel.email = :email 
                         AND novel.tags LIKE :tag 
                         GROUP BY novel.id 
...
?>
    ...
    <div class="pagination d-flex gap-1 justify-content-center mt-3">
      <?php if ($page > 1): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=popular&tag=<?php echo urlencode($tag); ?>&page=1"><i class="bi text-stroke bi-chevron-double-left"></i></a>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=popular&tag=<?php echo urlencode($tag); ?>&page=<?php echo $page - 1; ?>"><i class="bi text-stroke bi-chevron-left"></i></a>
      <?php endif; ?>
      ...
      <?php
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
          if ($i === $page) {
            ...
            echo '<a class="btn btn-sm btn-primary fw-bold" href="?by=popular&tag=' . urlencode($tag) . '&page=' . $i . '">' . $i . '</a>';
          }
        }
      ?>

      <?php if ($page < $totalPages): ?>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=popular&tag=<?php echo urlencode($tag); ?>&page=<?php echo $page + 1; ?>"><i class="bi text-stroke bi-chevron-right"></i></a>
        <a class="btn btn-sm btn-primary fw-bold" href="?by=popular&tag=<?php echo urlencode($tag); ?>&page=<?php echo $totalPages; ?>"><i class="bi text-stroke bi-chevron-double-right"></i></a>
      <?php endif; ?>
    </div>