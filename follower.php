...
if(isset($_GET['id'])){
  ...
  $stmt = $db->prepare("SELECT * FROM users WHERE email IN ('" . implode("','", $follower_users) . "') ORDER BY id DESC");
  $result = $stmt->execute();
...
<html>
  ...
  <body>
    ...
    <div class="input-group mb-3 mt-2">
      <input type="text" class="form-control me-2 ms-2" placeholder="Search user that follow you" id="search-input">
    </div>
      ...
      <div class="row">
        <?php while($row = $result->fetchArray()) { ?>
          <div class="col-md-3 col-sm-6">
            <a href="artist.php?id=<?php echo $row['id']; ?>" class="opacity-75 btn tag-button btn-secondary mb-2 fw-bold text-start w-100">
              <?php echo $row['artist']; ?>
            </a>
          </div>
        <?php } ?>
