...
class UserController {
  ...
  public function addData() {
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add'])) {
      ...
      header('Location: ../admin/index.php?page=news');
      exit();
    }
  }
...
?>
    ...
    <center><button type="button" class="btn btn-primary fw-bold mt-2" data-bs-toggle="modal" data-bs-target="#add">Add New News</button></center>
    <center>
          ...
          <?php foreach ($data as $row): ?>  
            <div class="card mb-3">
              <div class="card-header bg-primary text-white fw-bold">
                ...
                News <a href="?page=news&edit=<?= $row['id'] ?>" class="btn-sm text-white float-end"><i class="bi bi-pencil-fill"></i></a>
              </div>
              <div class="card-body text-secondary fw-bold">
                ...
                <a href="?page=news&delete=<?= $row['id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure?')"><i class="bi bi-trash"></i></a>
              </div>
            </div>
          <?php endforeach; ?> 
        </div>
