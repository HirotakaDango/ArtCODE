      <div id="category-<?php echo $category; ?>" class="category-section pt-5">
        ...
        <div class="row">
          <?php foreach ($currentcharacters as $tag => $count): ?>
            <?php
              // Check if the tag has any associated images
              $stmt = $db->prepare("SELECT * FROM images WHERE characters LIKE ? ORDER BY id DESC LIMIT 1");
              $stmt->bindValue(1, '%' . $tag . '%');
              $imageResult = $stmt->execute()->fetchArray(SQLITE3_ASSOC);
              if ($imageResult):
            ?>
            <div class="col-md-2 col-sm-5 px-0">
              <a href="/admin/images_section/keyword/?character=<?php echo str_replace('%27', "'", urlencode($tag)); ?>" class="m-1 d-block text-decoration-none">
