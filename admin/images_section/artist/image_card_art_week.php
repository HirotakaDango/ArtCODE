              ...
              <?php include($_SERVER['DOCUMENT_ROOT'] . '/rows_columns/image_counts.php'); ?>
                <div class="dropdown">
                  ...
                  <ul class="dropdown-menu">
                    <li><a class="dropdown-item fw-bold" href="/admin/images_section/edit/?id=<?php echo $imageW['id']; ?>&back=<?php echo urlencode('http' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']); ?>"><i class="bi bi-pencil-fill"></i> <small>edit</small></a></li>
                    <?php
                      $is_favorited = $db->query("SELECT COUNT(*) FROM favorites WHERE email = '{$_SESSION['email']}' AND image_id = {$imageW['id']}")->fetchColumn();
                      if ($is_favorited) {
                    ?>
                      <form method="POST">
                  ...
                  <?php include('share_artist_week.php'); ?>

                  <?php include($_SERVER['DOCUMENT_ROOT'] . '/admin/images_section/artist/components/card_image_week.php'); ?>
