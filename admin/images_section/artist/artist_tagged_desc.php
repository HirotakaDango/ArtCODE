...
if (isset($_GET['tag'])) {
  ...
  $stmt = $db->prepare("SELECT images.* FROM images JOIN users ON images.email = users.email WHERE users.id = :id AND images.tags LIKE :tagPattern ORDER BY images.id DESC LIMIT :limit OFFSET :offset");
  ...
  $stmt = $db->prepare("SELECT images.* FROM images JOIN users ON images.email = users.email WHERE users.id = :id ORDER BY images.id DESC LIMIT :limit OFFSET :offset");
  $stmt->bindParam(':id', $id);
  $stmt->bindValue(':limit', $limit, PDO::PARAM_INT);
  $stmt->bindValue(':offset', $offset, PDO::PARAM_INT);
}
...
?>
    ...
    <?php include('image_card_art_tagged_desc.php'); ?>