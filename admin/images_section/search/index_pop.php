...
$queryNum->bindValue(':email', $email, PDO::PARAM_STR); // Assuming $email is the email you want to search for
$queryNum->execute();
$user = $queryNum->fetch(PDO::FETCH_ASSOC);

$numpage = isset($user['numpage']) ? (int)$user['numpage'] : 50; // Ensure $numpage is an integer

// Determine the number of items per page
$limit  = $numpage > 0 ? $numpage : PHP_INT_MAX; // Avoid division by zero
...
if (empty($searchTerm)) {
  // If q is empty, order by favorite_count DESC
  $query .= " GROUP BY images.id 
              ORDER BY favorite_count DESC";
} else {
  // Otherwise, order by favorite_count DESC
  $query .= " GROUP BY images.id 
              ORDER BY favorite_count DESC";
}
