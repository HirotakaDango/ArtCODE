...
$stmt = $db->prepare("SELECT * FROM images WHERE REPLACE(`group`, ' ', '') LIKE :groupWithoutSpaces ESCAPE '\\' OR REPLACE(`group`, ' ', '') LIKE :group_start ESCAPE '\\' OR REPLACE(`group`, ' ', '') LIKE :group_end ESCAPE '\\' OR `group` = :group_exact ORDER BY view_count DESC LIMIT :limit OFFSET :offset");
$stmt->bindValue(':groupWithoutSpaces', "{$groupWithoutSpaces},%", SQLITE3_TEXT);
$stmt->bindValue(':group_start', "%,{$groupWithoutSpaces}", SQLITE3_TEXT);
$stmt->bindValue(':group_end', "%,{$groupWithoutSpaces},%", SQLITE3_TEXT);
$stmt->bindValue(':group_exact', $group, SQLITE3_TEXT);
