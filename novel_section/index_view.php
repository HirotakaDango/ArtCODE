...
try {
  $result = $db->prepare("SELECT novel.*, users.id AS user_id, users.email, users.artist FROM novel JOIN users ON novel.email = users.email ORDER BY novel.view_count DESC LIMIT :limit OFFSET :offset");
